...
#define DB_COPY_DB_STR_FROM(dst, src)                     \
  if (src) {                                              \
    ...
    if (len > (sizeof(dst) - 1)) len = (sizeof(dst) - 1); \
                                                          \
    if (len > 0) memcpy(dst, db_string->str, len);        \
                                                          \
    dst[len] = 0;                                         \
  } else {                                                \
    dst[0] = 0;                                           \
    ...
    while (                                                                    \
      ...
      switch (command) {                                                       \
        ...
        case DB_TECHNICIAN_READ_PHONE_NUMBER_COMMAND:                          \
          ...
          DB_DEBUG("\ttechnician_data->phone_number = %s",                     \
...
#define TECHNICIAN_MANIPULATE_HELPER(func, action, text)                       \
  ...
  if (technician_data) {                                                       \
    ...
    void *p;                                                                   \
    int err = 0;                                                               \
    while ((command = (DB_##action##_ENUM_COMMAND)va_arg(                      \
                args, DB_ADD_ENUM_COMMAND))) {                                 \
      p = (void *)va_arg(args, void *);                                        \
      switch (command) {                                                       \
        case DB_TECHNICIAN_##action##_ID_COMMAND:                              \
          ...
          DB_DEBUG(                                                            \
              "\ttechnician_data->id limit reached DB_TECHNICIAN_" #action     \
              "_ID_LIMIT_REACHED(%d)",                                         \
              DB_TECHNICIAN_##action##_ID_LIMIT_REACHED)                       \
...
#define CLIENT_DATA_MANIPULATE_HELPER(func, action, text)                     \
  ...
  if (client_data) {                                                          \
    ...
    int err = 0;                                                              \
    while ((command = (DB_##action##_ENUM_COMMAND)va_arg(                     \
                args, DB_##action##_ENUM_COMMAND))) {                         \
      p = (void *)va_arg(args, void *);                                       \
      switch (command) {                                                      \
        case DB_CLIENT_##action##_VERSION_COMMAND:                            \
          ssize_t version_as_ssize_t = (ssize_t)((intptr_t)p);                \
          if ((version_as_ssize_t <= (ssize_t)INT32_MAX) &&                   \
              (version_as_ssize_t >= (ssize_t)INT32_MIN)) {                   \
          ...
          DB_DEBUG("\tclient_data->version limit reached DB_CLIENT_" #action  \
                   "_VERSION_LIMIT_REACHED(%d)",                              \
                   DB_CLIENT_##action##_VERSION_LIMIT_REACHED)                \
        ...
        case DB_CLIENT_##action##_ID_COMMAND:                                 \
          ...
          DB_DEBUG("\tclient_data->id limit reached DB_CLIENT_" #action       \
                   "_ID_LIMIT_REACHED(%d)",                                   \
                   DB_CLIENT_##action##_ID_LIMIT_REACHED)                     \
        ...
        case DB_CLIENT_##action##_TECHNICIAN_ID_COMMAND:                      \
          ...
          if ((technician_id_as_ssize_t <= (ssize_t)INT32_MAX) &&             \
              (technician_id_as_ssize_t >= (ssize_t)INT32_MIN)) {             \
            client_data->technician_id = (int32_t)technician_id_as_ssize_t;   \
            touched |= CLIENT_TECHNICIAN_ID_COMMAND_TOUCHED;                  \
            DB_DEBUG("\tclient_data->technician_id = %d",                     \
                     client_data->technician_id)                              \
            break;                                                            \
          }                                                                   \
          DB_DEBUG(                                                           \
              "\tclient_data->technician_id limit reached DB_CLIENT_" #action \
              "_TECHNICIAN_ID_LIMIT_REACHED(%d)",                             \
              DB_CLIENT_##action##_TECHNICIAN_ID_LIMIT_REACHED)               \
        ...
        case DB_CLIENT_##action##_CREATED_AT_COMMAND:                         \
          ...
          DB_DEBUG("\tAdded client_data->created_at = %zu",                   \
                   (size_t)client_data->created_at)                           \
          break;                                                              \
        ...
        case DB_CLIENT_##action##_DISTRICT_CITY_COMMAND:                      \
          ...
          DB_DEBUG("\tclient_data->district_city = %s",                       \
                   client_data->district_city)                                \
          break;                                                              \
        ...
        case DB_CLIENT_##action##_PHONE_NUMBER_COMMAND:                       \
          ...
          DB_DEBUG("\tclient_data->phone_number = %s",                        \
                   client_data->phone_number)                                 \
          break;                                                              \
        default:                                                              \
          err = DB_CLIENT_##action##_INVALID_COMMAND;                         \
          DB_DEBUG("\tclient_data invalid command DB_CLIENT_" #action         \
                   "_INVALID_COMMAND(%d). Command: %d",                       \
                   DB_CLIENT_##action##_INVALID_COMMAND, command)             \
          goto func##_exit1;                                                  \
      }                                                                       \
    }                                                                         \
                                                                              \
...
#define CLIENT_DATA_READ_HELPER(func, text)                                  \
  ...
  if (client_data) {                                                         \
    ...
    int err = 0;                                                             \
    while ((command =                                                        \
                (DB_READ_ENUM_COMMAND)va_arg(args, DB_READ_ENUM_COMMAND))) { \
      p = (void *)va_arg(args, void *);                                      \
      switch (command) {                                                     \
        case DB_CLIENT_READ_VERSION_COMMAND:                                 \
          ...
          DB_DEBUG(                                                          \
              "\tclient_data->version limit reached "                        \
              "DB_CLIENT_READ_VERSION_LIMIT_REACHED(%d)",                    \
              DB_CLIENT_READ_VERSION_LIMIT_REACHED)                          \
        ...
        case DB_CLIENT_READ_ID_COMMAND:                                      \
          ...
          DB_DEBUG(                                                          \
              "\tclient_data->id limit reached "                             \
              "DB_CLIENT_READ_ID_LIMIT_REACHED(%d)",                         \
              DB_CLIENT_READ_ID_LIMIT_REACHED)                               \
        ...
        case DB_CLIENT_READ_TECHNICIAN_ID_COMMAND:                           \
          ...
          DB_DEBUG(                                                          \
              "\tclient_data->technician_id limit reached "                  \
              "DB_CLIENT_READ_TECHNICIAN_ID_LIMIT_REACHED(%d)",              \
              DB_CLIENT_READ_TECHNICIAN_ID_LIMIT_REACHED)                    \
        ...
        default:                                                             \
          ...
          DB_DEBUG(                                                          \
              "\tclient_data invalid command "                               \
              "DB_CLIENT_READ_INVALID_COMMAND(%d). Command: %d",             \
              DB_CLIENT_READ_INVALID_COMMAND, command)                       \
          goto func##_exit1;                                                 \
      }                                                                      \
    }                                                                        \
                                                                             \
