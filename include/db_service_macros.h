...
#define DB_BUILD_SQL(query_name, text, param_types_oid)               \
  ...
  res = PQprepare(conn, query_name, text,                             \
                  (int)(sizeof(param_types_oid) / sizeof(const Oid)), \
                  param_types_oid);                                   \
  ...
  if (res) {                                                          \
    if (PQresultStatus(res) != PGRES_COMMAND_OK) {                    \
      DB_ERROR(                                                       \
          "_db_init_query: PQresultStatus returned error for "        \
          "query " #query_name " with message: %s",                   \
...
#define DB_QUERY_AS_JSON(text, ch) "SELECT json_agg(" ch ") FROM (" text ") " ch
#endif
