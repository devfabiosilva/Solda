...
static void test_add_manipulation_destroy_on_failure(void *ctx) {
  ERROR_MSG_FMT("FAILURE: Destroying **technician_requests %p ...",
                (uintptr_t)ctx)
...
int test_add_manipulation() {
  ...
  if ((err = technician_acquire_technician_data_from_array(
           &technician_index, &technician_data, technician_requests)))
    goto test_add_manipulation_exit;

  printf("\ntechnician_data index %d and pointer %p", (int)technician_index,
         technician_data);
  ...
  err = TECHNICIAN_EXECUTE_ADD(
      technician_data, TECHNICIAN_ADD_NAME(expected_name_in_technician),
  ...
  C_ASSERT_EQUAL_U32(
      ...
      CTEST_SETTER(
          CTEST_TITLE("Touched field must be match with expected with %d.",
                      technician_touch_matched),
          CTEST_ON_ERROR_CB(test_add_manipulation_destroy_on_failure,
                            (void *)&technician_requests)))

  C_ASSERT_EQUAL_S32(
      0, technician_data->id,
      CTEST_SETTER(CTEST_TITLE("Check technician ID ..."),
                   CTEST_ON_ERROR_CB(test_add_manipulation_destroy_on_failure,
                                     (void *)&technician_requests),
                   CTEST_ON_ERROR(
                       "Failed tecnician id value. Was expected 0 but found %d",
                       technician_data->id)))

  C_ASSERT_EQUAL_S32(
      0, technician_data->version,
      CTEST_SETTER(
          CTEST_TITLE("Check technician version ..."),
          CTEST_ON_ERROR_CB(test_add_manipulation_destroy_on_failure,
                            (void *)&technician_requests),
          CTEST_ON_ERROR(
              "Failed tecnician version value. Was expected 0 but found %d",
              technician_data->version)))

  C_ASSERT_EQUAL_INT(
      IS_ROOT_ADMIN_SUPREME, technician_data->rules,
      CTEST_SETTER(CTEST_TITLE("Check technician rules ..."),
                   CTEST_ON_ERROR_CB(test_add_manipulation_destroy_on_failure,
                                     (void *)&technician_requests),
                   CTEST_ON_ERROR("Failed tecnician rules value. Was expected "
                                  "NO_ACCESS (0) but found %d",
                                  technician_data->version)))

  C_ASSERT_EQUAL_INT(
      TECHNICIAN_DATA_NEW, technician_data->flag,
      CTEST_SETTER(
          CTEST_TITLE("Check technician initialize in memory flag ..."),
          CTEST_ON_ERROR_CB(test_add_manipulation_destroy_on_failure,
                            (void *)&technician_requests),
          CTEST_ON_ERROR("Failed tecnician flag value. Was expected "
                         "TECHNICIAN_DATA_NEW (%d) but found %d",
                         TECHNICIAN_DATA_NEW, technician_data->version)))
  ...
  C_ASSERT_EQUAL_U64(
      ...
      CTEST_SETTER(
          ...
          CTEST_ON_ERROR_CB(test_add_manipulation_destroy_on_failure,
                            (void *)&technician_requests),
          CTEST_ON_ERROR("Failed. Was expected %lu but found %lu", 0U,
                         (uint64_t)technician_data->created_at)))
  ...
  C_ASSERT_EQUAL_STRING(
      ...
      CTEST_SETTER(
          ...
          CTEST_ON_ERROR_CB(test_add_manipulation_destroy_on_failure,
                            (void *)&technician_requests),
          CTEST_ON_ERROR("Failed. Was expected %s but found %s",
                         expected_name_in_technician, technician_data->name)))
  ...
  C_ASSERT_EQUAL_STRING(
      ...
      CTEST_SETTER(
          ...
          CTEST_ON_ERROR_CB(test_add_manipulation_destroy_on_failure,
                            (void *)&technician_requests),
          CTEST_ON_ERROR("Failed. Was expected %s but found %s",
                         expected_phone_number_in_technician,
                         technician_data->phone_number)))

  C_ASSERT_NULL(
      technician_data->client_requests.array,
      CTEST_SETTER(
          CTEST_TITLE("Checking client_requests.array field in Technician data "
                      "is NULL (lazy mode) ..."),
          CTEST_ON_ERROR_CB(test_add_manipulation_destroy_on_failure,
                            (void *)&technician_requests),
  ...
  C_ASSERT_EQUAL_U64(
      ...
      CTEST_SETTER(
          CTEST_TITLE(
              "Checking client_requests.n in technician data (lazy mode) ..."),
          CTEST_ON_ERROR_CB(test_add_manipulation_destroy_on_failure,
                            (void *)&technician_requests),
          CTEST_ON_ERROR("Failed. Was expected 0 size array but found %lu",
                         (uint64_t)technician_data->client_requests.n)))

  C_ASSERT_EQUAL_U64(
      0, (uint64_t)technician_data->client_requests.array_max_len,
      CTEST_SETTER(
          CTEST_TITLE("Checking client_requests.array_max_len in technician "
                      "data (lazy mode) ..."),
          CTEST_ON_ERROR_CB(test_add_manipulation_destroy_on_failure,
                            (void *)&technician_requests),
          CTEST_ON_ERROR(
              "Failed. Was expected 0 size array but found %lu",
              (uint64_t)technician_data->client_requests.array_max_len)))
  ...
  if ((err = technician_acquire_client_data_from_array(
           &client_data_index, &client_data, technician_index,
           technician_requests)))
    goto test_add_manipulation_exit;

  printf("\nclient_data index %d and pointer %p", (int)client_data_index,
         client_data);
  ...
  err = CLIENT_EXECUTE_ADD(
      client_data, CLIENT_ADD_TECHNICIAN_ID(technician_index),
      CLIENT_ADD_CPF(client_cpf), CLIENT_ADD_NAME(client_name),
  ...
  C_ASSERT_EQUAL_U32(
      ...
      CTEST_SETTER(
          ...
          CTEST_ON_ERROR_CB(test_add_manipulation_destroy_on_failure,
                            (void *)&technician_requests),
          CTEST_ON_ERROR("Failed. Was expected touched %d but found %d",
                         client_touch_matched, client_data->touched),
  ...
  C_ASSERT_EQUAL_S32(
      0, client_data->id,
      CTEST_SETTER(CTEST_TITLE("Checking id field on client data ..."),
                   CTEST_ON_ERROR_CB(test_add_manipulation_destroy_on_failure,
                                     (void *)&technician_requests),
                   CTEST_ON_ERROR("Failed. Was expected 0 but found %d",
                                  client_data->id),
                   CTEST_ON_SUCCESS("Test id field success for client data")))

  C_ASSERT_EQUAL_S32(
      0, client_data->technician_id,
      CTEST_SETTER(
          CTEST_TITLE("Checking technician_id field on client data ..."),
          CTEST_ON_ERROR_CB(test_add_manipulation_destroy_on_failure,
                            (void *)&technician_requests),
          CTEST_ON_ERROR("Failed. Was expected 0 but found %d",
                         client_data->technician_id),
          CTEST_ON_SUCCESS("Test technician_id field success for client data")))

  C_ASSERT_EQUAL_S32(
      0, client_data->version,
      CTEST_SETTER(
  ...
  C_ASSERT_EQUAL_STRING(
      ...
      CTEST_SETTER(CTEST_TITLE("Checking name field on client data ..."),
                   CTEST_ON_ERROR_CB(test_add_manipulation_destroy_on_failure,
                                     (void *)&technician_requests),
                   CTEST_ON_ERROR("Failed. Was expected %s but found %s",
                                  client_name, client_data->name),
  ...
  C_ASSERT_EQUAL_STRING(
      ...
      CTEST_SETTER(
          ...
          CTEST_ON_ERROR_CB(test_add_manipulation_destroy_on_failure,
                            (void *)&technician_requests),
          CTEST_ON_ERROR("Failed. Was expected %s but found %s", client_address,
                         client_data->address),
  ...
  C_ASSERT_EQUAL_STRING(
      ...
      CTEST_SETTER(
          ...
          CTEST_ON_ERROR_CB(test_add_manipulation_destroy_on_failure,
                            (void *)&technician_requests),
          CTEST_ON_ERROR("Failed. Was expected %s but found %s", client_email,
                         client_data->email),
  ...
  C_ASSERT_NULL(
      client_data->repair_requests.array,
      CTEST_SETTER(
          CTEST_TITLE("Checking repair_requests.array field in client data is "
                      "NULL (lazy mode) ..."),
          CTEST_ON_ERROR_CB(test_add_manipulation_destroy_on_failure,
                            (void *)&technician_requests),
          CTEST_ON_ERROR("Failed. Was expected NULL but NOT NULL found")))
  ...
  C_ASSERT_EQUAL_U64(
      ...
      CTEST_SETTER(
          CTEST_TITLE(
              "Checking repair_requests.n in client data (lazy mode) ..."),
          CTEST_ON_ERROR_CB(test_add_manipulation_destroy_on_failure,
                            (void *)&technician_requests),
          CTEST_ON_ERROR("Failed. Was expected 0 size array but found %lu",
                         (uint64_t)client_data->repair_requests.n)))

  C_ASSERT_EQUAL_U64(
      0, (uint64_t)client_data->repair_requests.array_max_len,
      CTEST_SETTER(
          CTEST_TITLE("Checking repair_requests.array_max_len in client data "
                      "(lazy mode) ..."),
          CTEST_ON_ERROR_CB(test_add_manipulation_destroy_on_failure,
                            (void *)&technician_requests),
          CTEST_ON_ERROR("Failed. Was expected 0 size array but found %lu",
                         (uint64_t)client_data->repair_requests.array_max_len)))

  size_t repair_request_index;
  REPAIR *repair = NULL;
  if ((err = technician_acquire_repair_request_from_array(
           &repair_request_index, &repair, client_data_index, technician_index,
           technician_requests)))
    goto test_add_manipulation_exit;
  ...
  err = REPAIR_EXECUTE_ADD(
      repair, REPAIR_ADD_CLIENT_ID(client_id), REPAIR_ADD_IS_BUDGET(is_budget),
      ...
      REPAIR_ADD_DELIVERY_DATE(delivery_date), REPAIR_ADD_WARRANTY(warranty))

      if (err) goto test_add_manipulation_exit;

  int32_t repair_touch_matched =
      REPAIR_CLIENT_ID_COMMAND_TOUCHED | REPAIR_IS_BUDGET_COMMAND_TOUCHED |
      REPAIR_DEVICE_PROBLEM_COMMAND_TOUCHED |
      REPAIR_BRAND_MODEL_COMMAND_TOUCHED | REPAIR_SERIAL_MODEL_COMMAND_TOUCHED |
      REPAIR_CLAIMED_DEFECT_COMMAND_TOUCHED |
      REPAIR_OBSERVATIONS_COMMAND_TOUCHED |
      REPAIR_MONETARY_TYPE_COMMAND_TOUCHED |
      REPAIR_EXPECTED_BUDGET_DATE_COMMAND_TOUCHED |
      REPAIR_EXPECTED_DELIVERY_DATE_DATE_COMMAND_TOUCHED |
      REPAIR_LABOR_BUDEGET_DATE_COMMAND_TOUCHED |
      REPAIR_DELIVERY_DATE_COMMAND_TOUCHED |
  ...
  C_ASSERT_EQUAL_S32(
      ...
      CTEST_SETTER(CTEST_TITLE("Checking client_id in repair ..."),
                   CTEST_ON_ERROR_CB(test_add_manipulation_destroy_on_failure,
                                     (void *)&technician_requests),
                   CTEST_ON_ERROR("Failed. Was expected %d but found %d",
                                  client_id, repair->client_id)))
  ...
  C_ASSERT_TRUE(
      ...
      CTEST_SETTER(
          ...
          CTEST_ON_ERROR_CB(test_add_manipulation_destroy_on_failure,
                            (void *)&technician_requests),
          CTEST_ON_ERROR(
              "Failed. Was expected repair->is_bugdet = %d but found %d",
              is_budget, (int)repair->is_bugdet)))

  C_ASSERT_EQUAL_INT(
      device_problem, repair->device_problem,
      CTEST_SETTER(CTEST_TITLE("Checking device_problem in repair ..."),
                   CTEST_ON_ERROR_CB(test_add_manipulation_destroy_on_failure,
                                     (void *)&technician_requests),
                   CTEST_ON_ERROR("Failed. Was expected %d but found %d",
                                  device_problem, repair->device_problem)))
  ...
  C_ASSERT_EQUAL_STRING(
      ...
      CTEST_SETTER(CTEST_TITLE("Checking brand_model in repair ..."),
                   CTEST_ON_ERROR_CB(test_add_manipulation_destroy_on_failure,
                                     (void *)&technician_requests),
                   CTEST_ON_ERROR("Failed. Was expected %s but found %s",
                                  brand_model, repair->brand_model)))
  ...
  C_ASSERT_EQUAL_STRING(
      ...
      CTEST_SETTER(CTEST_TITLE("Checking serial_number in repair ..."),
                   CTEST_ON_ERROR_CB(test_add_manipulation_destroy_on_failure,
                                     (void *)&technician_requests),
                   CTEST_ON_ERROR("Failed. Was expected %s but found %s",
                                  serial_number, repair->serial_number)))
  ...
  C_ASSERT_EQUAL_STRING(
      ...
      CTEST_SETTER(CTEST_TITLE("Checking claimed_defect in repair ..."),
                   CTEST_ON_ERROR_CB(test_add_manipulation_destroy_on_failure,
                                     (void *)&technician_requests),
                   CTEST_ON_ERROR("Failed. Was expected %s but found %s",
                                  claimed_defect, repair->claimed_defect)))
  ...
  C_ASSERT_EQUAL_STRING(
      ...
      CTEST_SETTER(CTEST_TITLE("Checking observation in repair ..."),
                   CTEST_ON_ERROR_CB(test_add_manipulation_destroy_on_failure,
                                     (void *)&technician_requests),
                   CTEST_ON_ERROR("Failed. Was expected %s but found %s",
                                  observation, repair->observations)))
  ...
  C_ASSERT_EQUAL_U64(
      ...
      CTEST_SETTER(
          ...
          CTEST_ON_ERROR_CB(test_add_manipulation_destroy_on_failure,
                            (void *)&technician_requests),
          CTEST_ON_ERROR("Failed. Was expected %lu but found %lu",
                         expected_budget_date, repair->expected_budget_date)))

  C_ASSERT_EQUAL_U64(
      expected_delivery_date, repair->expected_delivery_date,
      CTEST_SETTER(CTEST_TITLE("Checking expected_delivery_date in repair ..."),
                   CTEST_ON_ERROR_CB(test_add_manipulation_destroy_on_failure,
                                     (void *)&technician_requests),
                   CTEST_ON_ERROR("Failed. Was expected %lu but found %lu",
  ...
  C_ASSERT_EQUAL_S64(
      ...
      CTEST_SETTER(CTEST_TITLE("Checking labor_bugdet in repair ..."),
                   CTEST_ON_ERROR_CB(test_add_manipulation_destroy_on_failure,
                                     (void *)&technician_requests),
                   CTEST_ON_ERROR("Failed. Was expected %lu but found %lu",
                                  labor_budget, repair->labor_budget)))
  ...
  C_ASSERT_EQUAL_INT(
      ...
      CTEST_SETTER(CTEST_TITLE("Checking monetary_type in repair ..."),
                   CTEST_ON_ERROR_CB(test_add_manipulation_destroy_on_failure,
                                     (void *)&technician_requests),
                   CTEST_ON_ERROR("Failed. Was expected %d but found %d",
                                  monetary_type, repair->monetary_type)))
  ...
  C_ASSERT_EQUAL_U64(
      ...
      CTEST_SETTER(CTEST_TITLE("Checking delivery_date in repair ..."),
                   CTEST_ON_ERROR_CB(test_add_manipulation_destroy_on_failure,
                                     (void *)&technician_requests),
                   CTEST_ON_ERROR("Failed. Was expected %lu but found %lu",
                                  delivery_date, repair->delivery_date)))
  ...
  C_ASSERT_EQUAL_U64(
      ...
      CTEST_SETTER(CTEST_TITLE("Checking warranty in repair ..."),
                   CTEST_ON_ERROR_CB(test_add_manipulation_destroy_on_failure,
                                     (void *)&technician_requests),
                   CTEST_ON_ERROR("Failed. Was expected %lu but found %lu",
                                  warranty, repair->warranty)))
  ...
  if ((err = technician_acquire_service_request_from_array(
           &service_request_index, &service, client_data_index,
           technician_index, repair_request_index, technician_requests)))
    goto test_add_manipulation_exit;

  printf("\nservice index %d and pointer %p", (int)service_request_index,
         service);
  ...
  err = SERVICE_EXECUTE_ADD(service, SERVICE_REPAIR_REQUEST_ID(request_id),
                            SERVICE_QUANTITY(quantity),
                            SERVICE_MONETARY_TYPE(servic_monetary_type),
                            SERVICE_UNITY_PRICE(service_unity_price),
                            SERVICE_DESCRIPTION(service_description))

      if (err != 0) goto test_add_manipulation_exit;

  printf("\nValue of %p of %s\n", service, service->description);

  int32_t service_touch_matched = SERVICE_REPAIR_REQUEST_ID | SERVICE_QUANTITY |
                                  SERVICE_MONETARY_TYPE | SERVICE_UNITY_PRICE |
                                  SERVICE_DESCRIPTION;
...
int test_add_invalid_manipulation() {
  ...
  if (err) {
    printf(
        "\ntest_add_invalid_manipulation: technician_data_requests_init error "
        "%d\n",
        err);
    return err;
  }
  ...
  printf(
      "\ntest_add_invalid_manipulation: technician_data index %d and pointer "
      "%p",
      (int)technician_index, technician_data);
  ...
  if (err == DB_TECHNICIAN_ADD_INVALID_COMMAND)
    ...
    C_ASSERT_FAIL(CTEST_SETTER(
        CTEST_TITLE("technician_add failed to check invalid command ..."),
        CTEST_ON_ERROR_CB(test_add_manipulation_destroy_on_failure,
                          (void *)&technician_requests),
        CTEST_ON_ERROR("Failed. Was expected "
                       "DB_TECHNICIAN_ADD_INVALID_COMMAND(%d) but found %d",
                       DB_TECHNICIAN_ADD_INVALID_COMMAND, err)))
  }

test_add_invalid_manipulation_exit:
  printf(
      "\ntest_add_invalid_manipulation: technician_requests pointer before "
      "free %p\n",
      technician_requests);
  technician_data_requests_free(&technician_requests);
  printf(
      "\ntest_add_invalid_manipulation: technician_requests pointer after free "
      "%p\n",
      technician_requests);
