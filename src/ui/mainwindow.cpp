...
{
    ui->setupUi(this);
    connect(ui->loadDatabase, &QPushButton::clicked, this, &MainWindow::loadSoldaDatabase);
}
...
void MainWindow::loadSoldaDatabase()
{
    ...
    if (err == 0) {
        ...
        if (err == 0) {
            ...
            ui->tableView->horizontalHeader()->setSectionResizeMode(QHeaderView::Stretch); // Column stretch
            ui->tableView->setSelectionBehavior(QAbstractItemView::SelectRows);            // Select lines on click
