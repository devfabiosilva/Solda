...
void service_requests_clear(SERVICE_REQUESTS *service_requests) {
  if (service_requests) _service_requests_clear(service_requests);
}
...
void _repair_request_clear(REPAIR *request) {
  ...
  if (service_request_array)
    while (request->optional_service_requests.n > 0) {
      SERVICE *service_request_ptr =
          &request->optional_service_requests
               .array[--(request->optional_service_requests.n)];
...
void repair_request_clear(REPAIR *request) {
  if (request) _repair_request_clear(request);
}
...
void repair_requests_clear(REPAIR_REQUESTS *repair_requests) {
  if (repair_requests) _repair_requests_clear(repair_requests);
}
...
static void repair_requests_free(REPAIR_REQUESTS *repair_requests) {
  if (repair_requests->array) {
    ...
    while (array_max_len > 0) {
      ...
      SERVICE_REQUESTS *optional_service_requests =
          &repair_request_ptr->optional_service_requests;
      // Last child array: service request
      DB_ARRAY_FREE(optional_service_requests)
    }

    explicit_bzero(
        (void *)repair_requests->array,
        repair_requests->array_max_len * sizeof(*(repair_requests->array)));
...
static void client_data_requests_free(
    CLIENT_DATA_REQUESTS *client_data_request) {
  if (client_data_request->array) {
    ...
    while (array_max_len > 0) {
      CLIENT_DATA *client_data_ptr =
          &client_data_request->array[--array_max_len];
      repair_requests_free(&client_data_ptr->repair_requests);
    }

    explicit_bzero((void *)client_data_request->array,
                   client_data_request->array_max_len *
                       sizeof(*(client_data_request->array)));
...
void client_data_clear(CLIENT_DATA *client_data) {
  ...
  if (client_data) _client_data_clear(client_data);
}
...
int technician_data_requests_init(TECHNICIAN_DATA_REQUESTS **requests) {
  if (db_alloc((void **)requests, sizeof(**requests)) == 0) return 0;
...
void technician_data_requests_free(TECHNICIAN_DATA_REQUESTS **requests) {
  ...
  if ((requests != NULL) && (*requests != NULL)) {
    if ((*requests)->array) {
      ...
      while (array_max_len > 0) {
        TECHNICIAN_DATA *technician_data_ptr =
            &(*requests)->array[--array_max_len];
      ...
      explicit_bzero(
          (void *)(*requests)->array,
          (*requests)->array_max_len * sizeof(*((*requests)->array)));
...
void _technician_data_clear(TECHNICIAN_DATA *request) {
  ...
  if (client_data_array)
    while (request->client_requests.n > 0) {
      CLIENT_DATA *client_data_ptr =
          &request->client_requests.array[--(request->client_requests.n)];
...
void technician_data_clear(TECHNICIAN_DATA *request) {
  if (request) _technician_data_clear(request);
}
...
int technician_acquire_technician_data_from_array(
    size_t *index, TECHNICIAN_DATA **out, TECHNICIAN_DATA_REQUESTS *in) {
  int err = 0;
  ...
  if ((index != NULL) && (out != NULL) && (*out == NULL) && (in != NULL)) {
    ...
    *index = 0;
    ...
    if (in->array) {
      ...
      *out = &(in->array[in->n++]);
      DB_DEBUG(
          "technician_acquire_technician_data_from_array: Adding next index "
          "%zu in alloc'd array %p",
          in->n, in->array)

    } else if (db_alloc((void **)&(in->array),
                        MIN_TECHNICIAN_DATA_REQUESTS_INITIAL *
                            sizeof(*(in->array))) == 0) {
      DB_DEBUG(
          "Initialized array in technician_acquire_technician_data_from_array "
          "%p of size %zu",
          (void *)in->array,
          MIN_TECHNICIAN_DATA_REQUESTS_INITIAL * sizeof(*(in->array)))
  ...
  DB_DEBUG(
      "Exiting technician_acquire_technician_data_from_array with status code "
      "%d",
      err)
  return err;
}
...
int technician_acquire_client_data_requests_from_array(
    CLIENT_DATA_REQUESTS **out, size_t technician_index,
    TECHNICIAN_DATA_REQUESTS *in) {
  TECHNICIAN_ACQUIRE_CLIENT_DATA_REQUEST_FROM_ARRAY_BEGIN(
      (out != NULL) && (*out == NULL) && (in != NULL), *out = NULL;)
...
int technician_acquire_client_data_from_array(size_t *index, CLIENT_DATA **out,
  ...
  if (this_technician_data->client_requests.array) {
    DB_DEBUG(
        "Check this_technician_data->client_requests.n has reached limit >= "
        "%zu ...",
        this_technician_data->client_requests.array_max_len)
    if (this_technician_data->client_requests.n >=
        this_technician_data->client_requests.array_max_len) {
      DB_DEBUG(
          "Check this_technician_data->client_requests.n. Growing object in "
          "memory")
      int err =
          client_data_requests_grow(&this_technician_data->client_requests, 1);
      if (err) {
        DB_DEBUG("this_technician_data->client_requests.array grow failed %d",
                 err)
        return err;
      }
    ...
    *out = &this_technician_data->client_requests
                .array[this_technician_data->client_requests.n++];
    DB_DEBUG(
        "technician_acquire_client_data_from_array success with index %zu at "
        "pointer %p",
        *index, *out)
    return 0;
  }
  ...
  DB_DEBUG(
      "Check this_technician_data->client_requests.array is NULL. Alloc new "
      "one ...")
  if (db_alloc((void **)&(this_technician_data->client_requests.array),
               MIN_CLIENT_DATA_REQUESTS_INITIAL *
                   sizeof(*(this_technician_data->client_requests.array))) ==
      0) {
    this_technician_data->client_requests.array_max_len =
        MIN_CLIENT_DATA_REQUESTS_INITIAL;
    *index = 0;
    ...
    *out = &this_technician_data->client_requests.array[0];
    DB_DEBUG(
        "Check this_technician_data->client_requests.array created. Deliver "
        "index 0 at pointer %p",
        *out)
    return 0;
  }
  ...
  DB_DEBUG(
      "Error %d "
      "(DB_UNABLE_TO_ACQUIRE_AND_ALLOC_DATA_REQUESTS_FROM_TECHNICIAN_DATA_"
      "REQUESTS)",
      DB_UNABLE_TO_ACQUIRE_AND_ALLOC_DATA_REQUESTS_FROM_TECHNICIAN_DATA_REQUESTS)
  return DB_UNABLE_TO_ACQUIRE_AND_ALLOC_DATA_REQUESTS_FROM_TECHNICIAN_DATA_REQUESTS;
  TECHNICIAN_ACQUIRE_CLIENT_DATA_REQUEST_FROM_ARRAY_END
}

int technician_acquire_repair_requests_from_array(
    REPAIR_REQUESTS **out, size_t technician_index, size_t client_data_index,
    TECHNICIAN_DATA_REQUESTS *in) {
...
int technician_acquire_repair_request_from_array(size_t *index, REPAIR **out,
                                                 size_t technician_index,
  ...
  if (this_client_data->repair_requests.array) {
    if (this_client_data->repair_requests.n >=
        this_client_data->repair_requests.array_max_len) {
    ...
    *out = &this_client_data->repair_requests
                .array[this_client_data->repair_requests.n++];

    return 0;
  }

  if (db_alloc((void **)&(this_client_data->repair_requests.array),
               MIN_REPAIR_REQUESTS_INITIAL *
                   sizeof(*(this_client_data->repair_requests.array))) == 0) {
    this_client_data->repair_requests.array_max_len =
        MIN_REPAIR_REQUESTS_INITIAL;
    *index = 0;
...
int technician_acquire_service_requests_from_array(
    SERVICE_REQUESTS **out, size_t technician_index, size_t client_data_index,
    size_t repair_request_index, TECHNICIAN_DATA_REQUESTS *in) {
...
int technician_acquire_service_request_from_array(
    size_t *index, SERVICE **out, size_t technician_index,
    size_t client_data_index, size_t repair_request_index,
    TECHNICIAN_DATA_REQUESTS *in) {
  TECHNICIAN_ACQUIRE_SERVICE_REQUEST_FROM_ARRAY_BEGIN(
      (index != NULL) && (out != NULL) && (*out == NULL) && (in != NULL),
      *out = NULL;
  ...
  if (this_repair_request->optional_service_requests.array) {
    if (this_repair_request->optional_service_requests.n >=
        this_repair_request->optional_service_requests.array_max_len) {
      int err = service_request_grow(
          &this_repair_request->optional_service_requests, 1);
      if (err) return err;
    }

    *index = this_repair_request->optional_service_requests.n;
    *out = &this_repair_request->optional_service_requests
                .array[this_repair_request->optional_service_requests.n++];
    return 0;
  }

  if (db_alloc(
          (void **)&(this_repair_request->optional_service_requests.array),
          MIN_SERVICE_REQUESTS_INITIAL *
              sizeof(
                  *(this_repair_request->optional_service_requests.array))) ==
      0) {
    this_repair_request->optional_service_requests.array_max_len =
        MIN_SERVICE_REQUESTS_INITIAL;
    *index = 0;
